...
- **redos / resource-exhaustion** — the export-resolution port (default-function / nested-method driving) now *reaches* the vulnerable regex fn (brace-expansion, ansi-html, color-string were all applicable+live but `fired 0`), but recall is **0/6**: the driver's generic evil inputs (`"a".repeat(≤42)+"!"`) can't trigger real ReDoS regexes, which need a **tailored** attack (format prefix + long pump + failing suffix, e.g. `"hwb("+"1".repeat(50000)+"!"`). The regex-aware attack-string generator (**#139**) now lands (canary + novel fire via a tailored prefix+pump+failing-suffix), but real recall is gated one level deeper on broader ReDoS **detection** (**#141**): CATASTROPHIC_SHAPE only catches nested-quantifier `(a+)+`, not overlapping-char-class polynomial ReDoS like color-string's `[\d\.]+` — the redos analog of "function-shaped ≠ uniformly drivable": *detectable-regex ≠ generically-triggerable*.
